#line 1 "C:\\Users\\LENOVO\\OneDrive\\Documents\\C\\_VScode\\Matrix.h"
#ifndef matrix_h
#define matrix_h
using namespace std;
class I;
...
public:
    ...
    T det();
    T trace();
    T cofactor(int r, int c);
    T minor(int r, int c);
    T &operator()(int r, int c);
    template <typename U>
    friend istream &operator>>(istream &in, const Matrix<U> &m);
    template <typename U>
    friend ostream &operator<<(ostream &out, const Matrix<U> &m);
    template <typename U>
    friend Matrix pow(const Matrix<U> &m, const int n);
    friend class I;
};
class I : public Matrix<int>
{
public:
    I(int n) : Matrix<int>(n, n)
    {
        for (int i = 0; i < n; i++)
        {
            arr[i][i] = 1;
...
template <typename U>
istream &operator>>(istream &in, const Matrix<U> &m)
{
    for (int i = 0; i < m.row; i++)
    {
        for (int j = 0; j < m.col; j++)
...
template <typename U>
Matrix<U> pow(const Matrix<U> &m, const int n)
{
    try
    {
        if (m.row != m.col)

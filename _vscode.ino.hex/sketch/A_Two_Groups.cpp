#line 1 "C:\\Users\\LENOVO\\OneDrive\\Documents\\C\\_VScode\\A_Two_Groups.cpp"
#include<bits/stdc++.h>
using namespace std;
...
int main()
{
    ...
    while(t--)
    {
        int n,ind=0;
        cin>>n;
        ll a[n],sumneg=0,ans,sum=0,y=0,sumpos=0;
        vl neg;
        neg.pb(0);
        f(i,0,n,1) 
        {
        ...
        vsort(neg);
        ll x=neg.back();
        if(sumneg==0 || sumneg==sum) ans=sum;
        else 
        {
            ans=max(abs(sumpos-sumneg),sum+x);
        }
        cout<<ans<<endl;
        }
    return 0;
